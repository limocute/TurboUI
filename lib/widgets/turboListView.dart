...
class TurboListView extends StatefulWidget{
  ...
  assert(!(showGroupTag != null && showGroupTag == true && (groupTagHeight == null || groupTagHeight <= 0.0)),
    '当显示组标签时groupTagHeight必填且值>0.0'),//当显示组标签时groupTagHeight必填且值>0.0
  assert(!(showGroupTag != null && showGroupTag == true && (itemExtent == null || itemExtent <= 0.0)),
    '当显示组标签时itemExtent必填且值>0.0'),//当显示组标签时itemExtent必填且值>0.0
  assert(!(showIndexBar != null && showIndexBar == true && (itemExtent == null || itemExtent <= 0.0)),
  ...
  /// indexbar提示 通知器
  final IndexBarHintChangeNotifier _indexBarHintChangeNotifier = IndexBarHintChangeNotifier();
  /// indexbar选中项 通知器
  final IndexBarSelectionChangeNotifier _indexBarSelectNotifier = IndexBarSelectionChangeNotifier();
...
class _TurboListViewState extends State<TurboListView>{
    ...
    if(widget.showHangTag == false)
    {
      ...
      widget.controller.addListener(() {
          ...
          if (_index != -1 && _lastIndex != _index) {
            _lastIndex = _index;
            // 通知IndexBar当前所在分组
            widget._indexBarSelectNotifier.notify(_groupTagLoaclMap.keys.toList()[_index]);
          }
        }
      );
    }
  ...
  void _initData() {
    ...
    if (list != null && list.isNotEmpty) {
      if( widget.sortDataByGroupTag)
        TurboListViewUtil.sortListByGroupTag(list);
    ...
    {
      TurboListViewUtil.setShowGroupTagStatus(_dataList,_groupTagLoaclMap,widget.header,widget.groupTagHeight,widget.itemExtent,widget.showGroupTag);
      groupTagLoaclMapList..clear()..addAll(_groupTagLoaclMap.values);
    }
    ...
    if (widget.useRealIndex) {
     ...
     //IndexBar给默认选中第一个
    if(widget._indexBarSelectNotifier.selectTag == null || widget._indexBarSelectNotifier.selectTag == ""){
      if(_groupTagLoaclMap != null && _groupTagLoaclMap.length != 0){
        widget._indexBarSelectNotifier.notify(_groupTagLoaclMap.keys.toList()[0]);
      }
    }
  }
  ...
  Widget _buildListView()
  {
    return  ListView.builder(
            ...
            itemExtent: widget.itemExtent != null && !widget.showGroupTag && widget.header == null ? widget.itemExtent : null,
  ...
  Widget _buildGroupTagItem(String currentGroupTag)
  {
    if (widget.groupTagItemBuilder != null) {
        ...
        _groupTagItem = DefaultGroupTagWidget(curTag:'$currentGroupTag',maxHeight: widget.groupTagHeight);
      }
  ...
  void _onIndexBarTouch(IndexBarDetails model) {
    ...
    widget._indexBarHintChangeNotifier.notify(model.tag,model.isTouch,model.position-topLeftPosition.dy-15.0);
